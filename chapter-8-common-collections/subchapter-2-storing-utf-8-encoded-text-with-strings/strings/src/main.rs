fn main() {
    // Besides the commonly used string slice type (&str) for string literals and borrows, Rust also offers a truely
    // mutuable and UTF-8 encoded String type Strings borrow many aspects from vector like their creation
    ...
    _str = (5 * 5).to_string();
    // The big advantage of strings in Rust is their UTF-8 encoding, which allows fancy things like this without any
    // additional code
    let greetings = [
    ...
    hello_world += "d!";
    println!("{hello_world}");
    // You can also combine strings with it, but keep in mind that it takes ownership by default (str1 can not be used
    // afterwards)
    ...
    println!("{str5}");
    // Simple indexing to get a letter by doing &str[0] is not allowed though, as due to the UTF-8 encoding some
    // characters take up more than 1 byte of memory: 5 characters that use 5 bytes of memory
